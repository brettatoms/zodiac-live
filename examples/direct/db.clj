...
(defn add-message!
  "Stores a ciphertext body and its IV, returning the new row id.

  Returning the id is the one change from the framework version's copy: a targeted
  push renders exactly this message, so it needs to know which row it just wrote.
  The framework version never needed it, because a hint made every viewer re-derive
  the whole page and the diff found the new row on its own."
  [db channel-id author ciphertext iv]
  (:id (z.sql/execute-one! db {:insert-into :messages
